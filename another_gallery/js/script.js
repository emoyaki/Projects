$(document).ready(function () {
    ...
    $.get('xml/content.xml', function (load) {
        $(load).find('slide').each(function () {
            var $project = $(this);
            var name = $project.find('name').text();
            var description = $project.find('description').text();
            var image = $project.find('image').text();
            $('.image_container').append(image);
            $('.websites_name_content').append('<h1 class="name_website">' + name + '</h1>');
            $('.websites_description_content').append('<p class="description_website">' + description + '</p>');
        });
